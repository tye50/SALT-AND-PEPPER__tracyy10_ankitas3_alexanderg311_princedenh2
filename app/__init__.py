...
def analyze():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            t2d= text2data(text)
            senti=t2d.get("Sentiment")
            mag=t2d.get("Magnitude")
            obj=t2d.get("Objectivity")
            theme=t2d.get("Theme")
            key=t2d.get("Keywords")
            p = news_classifier.get_probability(model, text)
            return render_template("analyze.html", link=link, text=text, p_true=p[0], p_false=p[1], senti=senti, mag=mag, obj=obj, theme=theme, key=key)
